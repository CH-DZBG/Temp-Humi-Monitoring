...
const long PublishInterval = 5000; // 5秒
const long statusPublishInterval = 60000; // 60秒
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, topic_command) == 0) {
    if (cmd == "GET_TEMP" || cmd == "GET_HUMI" || cmd == "GET_ALL") {
      // 立即读取传感器并发布数据
      ...
      // LED控制逻辑
      digitalWrite(LED_BUILTIN, LOW);  // ESP8266 LED是低电平有效
...
void readSensorImmediately() {
  ...
  if (isnan(h) || isnan(t)) {
    sensorOK = false;
    Serial.println("❌ 立即读取DHT22失败!");
  } else {
    sensorOK = true;
    temp = t;
    hum = h;
    lastRead = millis();  // 更新最后读取时间
...
// 发布传感器数据函数，支持立即发布
void publishSensorData(bool immediate = false) {
  unsigned long currentTime = millis();

  // 如果是立即发布或者达到发布间隔
  if (immediate || (currentTime - lastPublish >= publishInterval)) {

    if (!mqttClient.connected()) {
      reconnect();
    }

    if (mqttClient.connected() && sensorOK) {
      ...
      if (mqttClient.publish(topic_data, buffer)) {
        publishCount++;
        Serial.println("✅ 数据已发布" + String(immediate ? " (立即)" : " (定时)"));
        Serial.print("📤 主题: ");
...
void publishDeviceStatus() {
  ...
  doc["device"] = deviceId;                                 // 设备唯一标识符
  doc["ip"] = WiFi.localIP().toString();                    // 当前IP地址
  doc["mac"] = WiFi.macAddress();                           // 设备物理地址
  doc["rssi"] = WiFi.RSSI();                                // WiFi信号强度（数值越小信号越好）
  doc["uptime"] = millis() / 1000;                          // 设备运行时长（秒）
  doc["publish_count"] = publishCount;                      // 累计发布次数
  doc["temperature"] = temp;                                // 当前温度值
  doc["humidity"] = hum;                                    // 当前湿度值
  doc["sensor_connected"] = sensorOK;                       // 传感器是否正常工作
  doc["wifi_connected"] = (WiFi.status() == WL_CONNECTED);  // WiFi是否已连接
  doc["mqtt_connected"] = mqttClient.connected();           // MQTT是否已连接
  doc["free_heap"] = ESP.getFreeHeap();                     // 剩余可用内存（字节）
...
<div class="status )rawliteral"
                + String(sensorOK ? "status-ok" : "status-error") + R"rawliteral(">
传感器状态: <strong>)rawliteral"
                + String(sensorOK ? "正常" : "异常") + R"rawliteral(</strong> | 
MQTT: <strong>)rawliteral"
                + String(mqttClient.connected() ? "已连接" : "未连接") + R"rawliteral(</strong>
</div>

<div class="data-box">
<div class="data-card temp">
<div>🌡️ 温度</div>
<div class="data-value">)rawliteral"
                + (sensorOK ? String(temp, 1) + "°C" : "N/A") + R"rawliteral(</div>
</div>
<div class="data-card hum">
<div>💧 湿度</div>
<div class="data-value">)rawliteral"
                + (sensorOK ? String(hum, 1) + "%" : "N/A") + R"rawliteral(</div>
</div>
</div>

<div class="info">
<h3>📊 设备信息</h3>
<p><strong>设备ID:</strong> )rawliteral"
                + deviceId + R"rawliteral(</p>
<p><strong>IP地址:</strong> )rawliteral"
                + WiFi.localIP().toString() + R"rawliteral(</p>
<p><strong>WiFi信号:</strong> )rawliteral"
                + String(WiFi.RSSI()) + R"rawliteral( dBm</p>
<p><strong>运行时间:</strong> )rawliteral"
                + String(millis() / 1000) + R"rawliteral( 秒</p>
<p><strong>发布次数:</strong> )rawliteral"
                + String(publishCount) + R"rawliteral(</p>
<p><strong>空闲内存:</strong> )rawliteral"
                + String(ESP.getFreeHeap()) + R"rawliteral( 字节</p>
</div>
...
<p>📶 数据每10秒自动发布到MQTT | 🔄 最后更新: )rawliteral"
                + String(millis() / 1000) + R"rawliteral( 秒前</p>
<p>📱 使用浏览器访问 <a href="http://)rawliteral"
                + WiFi.localIP().toString() + R"rawliteral(" target="_blank">网页监控界面</a> 获得更佳体验</p>
</div>
</div>
...
void handleCommand() {
  ...
  if (cmd == "GET_ALL" || cmd == "GET_TEMP" || cmd == "GET_HUMI") {
    readSensorImmediately();
...
void loop() {
  ...
  if (!mqttClient.connected()) {
    ...
    // 发布数据
    // 检查是否需要立即发布（命令触发后5秒内）
    if (publishImmediately && (millis() - lastCommandTime < PublishInterval)) {
      publishSensorData(true);
